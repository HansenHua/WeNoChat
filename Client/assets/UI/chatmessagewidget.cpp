...
void ChatMessageWidget::setFile(QString filePath,QString showName,QString time, QSize allSize,ChatMessageWidget::User_Type userType, QImage *image){
    m_msg = showName;
    m_userType = userType;
    m_time = time;
    m_filePath = filePath;
...
QSize ChatMessageWidget::fontRect(QString str)
{
    m_msg = str;
    int minHei = 30;
    int iconWH = 40;
    ...
    //    int widthWithoutListWidget = 600;
...
void ChatMessageWidget::paintEvent(QPaintEvent *event)
{
    ...
    if (m_userType == User_Type::User_She || m_userType == User_Type::File_She)
    { // 用户
        //头像
    ...
    else if (m_userType == User_Type::User_Me ||m_userType == User_Type::File_Me)
    { // 自己
        //头像
...
void ChatMessageWidget::mouseDoubleClickEvent(QMouseEvent *e)
{
    ...
    }else if (m_userType == File_Me || m_userType == File_She){
        QProcess process;
